...
import { Plus, Search, Filter, Music, Clock, Star, Trash2, Pencil, Download, Play, Pause, GripVertical, Save, X } from 'lucide-react';
...
export default function MeditationsPage() {
  ...
  const reorderMutation = useReorderMeditations();
  ...
  // Audio playback
  const togglePlay = (meditation: Meditation) => {
  ...
  const handleDragStart = (e: React.DragEvent, id: string) => {
    e.dataTransfer.setData('meditationId', id);
  ...
  const handleDragOver = (e: React.DragEvent) => {
    e.preventDefault();
    (e.currentTarget as HTMLElement).classList.add('ring-2', 'ring-purple-400');
  };

  const handleDragLeave = (e: React.DragEvent) => {
    (e.currentTarget as HTMLElement).classList.remove('ring-2', 'ring-purple-400');
  };
  ...
  const handleDrop = (e: React.DragEvent, targetId: string) => {
    ...
    const draggedId = e.dataTransfer.getData('meditationId');
  ...
  const enterReorderMode = () => {
    setOrderedIds(meditations.map(m => m.id));
    setReorderMode(true);
  };
  ...
  const saveReorder = async () => {
    try {
      ...
      toast({ title: t('success'), description: t('reorderSaved') });
  ...
  return (
    <div className="space-y-6">
          ...
          {!reorderMode ? (
            ...
            <>
              ...
              <Button onClick={saveReorder} disabled={reorderMutation.isPending}>
                ...
                {reorderMutation.isPending ? t('saving') : t('saveOrder')}
              </Button>
            </>
          )}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {(reorderMode
            ? (orderedIds.map(id => meditations.find(m => m.id === id)).filter((m): m is Meditation => m !== undefined))
            : meditations
          ).map((meditation) => (
            <Card
              key={meditation.id}
              className={`overflow-hidden transition-all ${reorderMode ? 'cursor-grab active:cursor-grabbing hover:ring-2 hover:ring-purple-400' : ''}`}
              draggable={reorderMode}
              onDragStart={(e) => handleDragStart(e, meditation.id)}
              onDragEnd={handleDragEnd}
              onDragOver={reorderMode ? handleDragOver : undefined}
              onDragLeave={reorderMode ? handleDragLeave : undefined}
              onDrop={reorderMode ? (e) => handleDrop(e, meditation.id) : undefined}
            >
              <div className="relative h-48 bg-gradient-to-br from-purple-500/20 to-blue-500/20">
                {reorderMode && (
                  <div className="absolute top-2 left-2 z-10 bg-black/50 rounded-full p-1.5">
                ...
                {meditation.audio_url && (
                  <button
                    onClick={(e) => { e.stopPropagation(); togglePlay(meditation); }}
              ...
              <CardContent className="p-4">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    ...
                    {playingId === meditation.id && (
                      <div className="flex items-center gap-1 text-purple-500 animate-pulse">
