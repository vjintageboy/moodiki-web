...
import { useReportedPosts, useDismissReports } from '@/hooks/use-reports';
import { format } from 'date-fns';

const categories = ['community', 'tips', 'success-story', 'question', 'discussion', 'meditation'];
...
export default function PostsPage() {
  ...
  const handleDismissReports = async (postId: string) => {
    try {
      await dismissReports.mutateAsync(postId);
      toast({ title: t('toasts.success'), description: t('toasts.reportsDismissed') });
    } catch (error: any) {
      toast({ title: t('toasts.error'), description: error.message, variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                    ...
                    {reportedPosts.some(rp => rp.id === post.id) && (
                      <Badge variant="outline" className="border-red-400 text-red-500 bg-red-50 dark:bg-red-950/30">
                        ...
                        {t('post.reported')} ({reportedPosts.find(rp => rp.id === post.id)?.reportCount || 1})
                      </Badge>
                    )}
                  ...
                  <div className="flex gap-2 pt-2 border-t flex-wrap">
                    ...
                    {reportedPosts.some(rp => rp.id === post.id) && (
                      <Button
                        ...
                        disabled={dismissReports.isPending}
                      >
                        {t('actions.dismissReports')}
                      </Button>
                    )}
                    <Button
                      size="sm"
                  ...
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {comments.map((comment: any) => {
                      const parentComment = comment.parent_comment_id 
                        ? comments.find((c: any) => c.id === comment.parent_comment_id)
                        : null;
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between mb-1 gap-2">
                            <div>
                              {parentComment && (
                                <div className="flex items-center gap-1 text-xs text-muted-foreground mb-1">
                                  ...
                                  <span>
                                    {t('dialogs.replyingTo', { 
                                      name: parentComment.is_anonymous 
                                        ? t('post.anonymous') 
                                        : parentComment.user?.full_name || t('post.unknownUser') 
                                    })}
                                  </span>
                                </div>
                              )}
                            ...
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-6 px-2 text-red-500 shrink-0"
      ...
      <AlertDialog open={!!deleteCommentId} onOpenChange={(open) => {
        if (!open && !deleteCommentMutation.isPending) setDeleteCommentId(null);
      }}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteCommentMutation.isPending}>{t('dialogs.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                handleDeleteComment();
