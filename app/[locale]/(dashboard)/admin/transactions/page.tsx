...
export default function TransactionsPage() {
    ...
    },
    {
      ...
      render: (tx) => (
        <Badge variant="outline" className={cn(
          ...
          {tx.payment_status === 'paid' ? <CheckCircle2 className="h-3 w-3" /> : <AlertCircle className="h-3 w-3" />}
          {tx.payment_status === 'paid' ? t('status.paid') : tx.payment_status === 'refunded' ? t('status.refunded') : tx.payment_status}
        </Badge>
      )
    },
    {
      ...
      render: (tx) => (
        tx.payment_status === 'paid' ? (
          <Button
            size="sm"
            variant="outline"
            className="text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200 h-8 gap-1"
  ...
  return (
    <div className="space-y-6 max-w-full">
      ...
      <AlertDialog open={!!refundConfirmId} onOpenChange={(open) => !open && setRefundConfirmId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isRefunding}>{t('refund.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => refundConfirmId && handleRefund(refundConfirmId)}
              disabled={isRefunding}
              className="bg-red-600 hover:bg-red-700"
            >
              {isRefunding ? (
                <><Loader2 className="h-4 w-4 mr-2 animate-spin" />{t('refund.processing')}</>
              ) : (
                <>{t('refund.confirm')}</>
              )}
            </AlertDialogAction>
